...
weatherplot1 <- ggplot(fixed_effects_weather_df, aes(x = month, y = estimate, color = is_significant)) +
  ...
  labs(
    title = "Monthly Weather Effects Impact Corn Yield",
    x = "Month",
    y = "Change in Corn Yield (bu/acre)",
    caption = "Figure 1. Estimated change in corn yield (bu/acre) for a 1-unit increase in monthly
temperature (°F) or precipitation (in), based on fixed effects from the mixed-effects model. Average bu/acre is 149.
Error bars represent 95% confidence intervals; red indicates statistically significant effects (p < 0.05).
Note: The two graphs use different y-axis scales, so effect magnitudes should be interpreted within each panel."
  ) +
...
#ggsave("visuals/weather_effects.png", plot = weatherplot1, width = 7, height = 5, units = "in")
```
