...
fixed_effects_mgt_df <- fixed_effects_mgt_df %>%
  mutate(
    term = recode(term,
                  ...
                  "areaOperatedAcresOperationMedian_scaled" = "Median Acres/Operation (Size, Scaled)",
                  "machineryAssetValueZ" = "Machinery Asset Value (Z-Score)"),
...
figure2<- ggplot(fixed_effects_mgt_df, aes(x = estimate, 
    ...
    labs(
    title = "Farm differences have Varrying Impacts on Corn Yield ",
    x = "Coefficient Estimate (Change in SD of Corn Yield)",
    y = NULL,
    caption = "Figure 2. The effect of a 1-standard deviation increase in each management variable on corn yield.
    Only variables with data prior to 2017 used.
    Points show coefficient estimate; bars show 95% Confidence Interval (LME Mode). Controlled for weather.
    Red indicates statistically significant effects (p < 0.05). All variables are scaled/z-scored."
  ) +
...
#ggsave("visuals/management_effects_plot.png", plot = figure2, width = 9, height = 3.5, units = "in")
```
...
fixed_effects_mgt_df2 <- fixed_effects_mgt_df2 %>%
  mutate(
    term = recode(term,
                  "areaNoTill_scaled" = "Area No-Till (Conservation, Scaled)",
                  "avgProducerAge_scaled" = "Average Producer Age (Demographics, Scaled)",
                  "CropAcresOperation_scaled" = "Crop Acres per Operation (Intensity, Scaled)",
                  "avgYearsOnPresentOperation_scaled" = "Years on Present Operation (Experience, Scaled)"),
...
figure3 <- ggplot(fixed_effects_mgt_df2, aes(x = estimate, 
  ...
  scale_color_manual(values = base_colors) +
 labs(
    title = "Farm differences with less data have 
less effect on Corn Yield ",
    x = "Coefficient Estimate (Change in SD of Corn Yield)",
    y = NULL,
    caption = "Figure 3. The effect of a 1-standard deviation increase in each management variable on corn yield.
    Only variables with data prior to 2017 used.
    Points show coefficient estimate; bars show 95% Confidence Interval (LME Mode). Controlled for weather.
...
#ggsave("visuals/management_effects_plot2.png", plot = figure3, width = 9, height = 3.5, units = "in")
